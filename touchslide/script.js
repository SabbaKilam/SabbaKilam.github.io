...
window.onload = function(){
	...
	function attachElements(){
        ...
        $.holder = $.id("holder");
        $.hiddenSlider = $.id("hiddenSlider");
        $.proxySlider = $.id("proxySlider");
        $.showPosition = $.id("showPosition");
	}
	...
	$.hiddenSlider.onmouseout = function(){
		$.styles($.proxySlider)
			("font-size", "1rem")
			("opacity", "1")
		;
	...
	function monitorModel(){
	    setInterval(function(){
    	    ...
    	    var borderColor = " hsl(" + (100*(1-$.player.volume)) +", 100%, 30%)";
            $.styles($.volumeSlider) 
                ("border-left", volumeLeftBorder + "px solid " + borderColor)
                ("color", "black")
